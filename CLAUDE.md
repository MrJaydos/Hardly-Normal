...
- **OCR capture (`ocrBtn`, the "Aa" header button)** exists for price-ticket numbers that aren't barcodes at all — plain printed digits elsewhere on the ticket that the barcode itself doesn't resolve to a working search on the Harvey Norman site. Lazy-loads `tesseract.js@5` from a CDN only on first tap (same lazy pattern as the ZXing fallback), same reasoning: it's several MB and most scans never need it. It's a one-shot manual capture-and-recognize (`captureFullFrame()` → `worker.recognize()`), not a continuous per-frame loop like barcode scanning — OCR is much slower than barcode decode, and unlike the barcode reticle there's no fixed on-screen position for this number, so the user frames it themselves and taps to capture. `tessedit_char_whitelist` is currently locked to digits `0-9` since every ticket code seen so far is numeric; loosen it if an alphanumeric one turns up. **OCR accuracy on a real ticket hasn't been verified against a physical device from this environment** — there's no camera to test with here, only the library's own published API contract. `#code`'s editability exists specifically so a misread is a quick fix, not a dead end.
