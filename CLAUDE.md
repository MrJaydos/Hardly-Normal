...
- **The ZXing fallback path decodes via `zxing.decode(canvasEl)`, not `decodeFromCanvas`** — that method doesn't exist on `@zxing/library@0.21.3`'s `BrowserCodeReader` and calling it throws on every frame. This previously broke scanning silently on any browser without native `BarcodeDetector` (all of iOS Safari, notably — Chrome/Android has the native API and was unaffected). If you touch `readFrame()`, check the actual method exists on the pinned ZXing version before assuming an API from docs/examples — sources vary by version, and the failure mode here (loop keeps running, camera looks fine, exception swallowed by the surrounding `catch`) gives no visible signal that anything is wrong.
